...
public class RecommendFragment extends BaseRVFragment<RecommendPresenter, Recommend.RecommendBooks> implements RecommendContract.View, RecyclerArrayAdapter.OnItemLongClickListener {
    ...
    @Override
    public void showBookToc(String bookId, List<BookMixAToc.mixToc.Chapters> list, String bookName, boolean isAppend) {
        chaptersList.clear();
        chaptersList.addAll(list);
        DownloadBookService.post(new DownloadQueue(bookId, list, 1, list.size(),bookName,isAppend));
        dismissDialog();
    }
                    ...
                    switch (which) {
                        ...
                        case 6:
                            ...
                            if (mAdapter.getItem(position).isFromSD) {
                                ...
                                showDialog();
                                mPresenter.getTocList(mAdapter.getItem(position)._id,mAdapter.getItem(position).title,true);
                            }
                            break;
                        default:
                            break;
